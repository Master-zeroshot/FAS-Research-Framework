...
data = dict(batch_size=50, data_loader_workers=8, sampler=True, pin_memory=True)

resize = dict(height=224, width=224)

checkpoint = dict(
    snapshot_name="mnv3_large_224_gausian_80_20_epoch20.pth.tar",
    experiment_path=get_log_path("logs_xgb"),
)
...
model = dict(
    ...
    embeding_dim=1280,
    imagenet_weights=get_model_path("mnv3_large_custom"),
)

aug = dict(type_aug=None, alpha=0.5, beta=0.5, aug_prob=0.7)

curves = dict(det_curve="det_curve_large.png", roc_curve="roc_curve_large.png")
