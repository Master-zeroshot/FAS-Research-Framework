...
data = dict(batch_size=48, data_loader_workers=8, sampler=True, pin_memory=True)

resize = dict(height=224, width=224)

checkpoint = dict(
    snapshot_name="mobilenetv4_medium_224_antispoof.pth.tar",
    experiment_path="./logs_mobilenetv4_medium",
)
...
model = dict(
    ...
    embeding_dim=1152,
    imagenet_weights="./pretrained/mobilenetv4_medium_imagenet.pth.tar",
)

aug = dict(type_aug=None, alpha=0.5, beta=0.5, aug_prob=0.7)

curves = dict(
    det_curve="det_curve_mobilenetv4_medium.png",
    roc_curve="roc_curve_mobilenetv4_medium.png",
)
...
test_file_name = "test_mobilenetv4_medium"
