...
datasets = dict(
    LCCFASD_root="./LCC_FASDcropped", Celeba_root="./CelebA_Spoof", Casia_root="./CASIA"
)
...
data = dict(batch_size=256, data_loader_workers=4, sampler=None, pin_memory=True)

resize = dict(height=128, width=128)

checkpoint = dict(snapshot_name="MobileNet3.pth.tar", experiment_path="./logs")
...
loss = dict(
    ...
    soft_triple=dict(cN=2, K=10, s=1, tau=0.2, m=0.35),
)
...
curves = dict(det_curve="det_curve_0.png", roc_curve="roc_curve_0.png")

dropout = dict(prob_dropout=0.1, classifier=0.1, type="bernoulli", mu=0.5, sigma=0.3)

data_parallel = dict(
    use_parallel=False, parallel_params=dict(device_ids=[0, 1], output_device=0)
)
